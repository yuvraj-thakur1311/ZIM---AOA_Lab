...
getOrderList();
  ...
  const hendatOrderDetailAction = (orderId:string)=>{
    const getOrderList = async ()=>{
    try{
            const responce = await fetch(`http://localhost:3000/orders/orderInfo?order_id=${orderId}`,{
            method:"GET",
            headers:{
                "Content-Type":"application/json"
            }
        ...
        <TableBody className="text-center">
          {orders.map((order) => (
            ...
            <TableRow
              ...
              className=""
            >
              <TableCell className="font-medium text-left">{order.order_id}</TableCell>
              <TableCell className="text-left">{order.case_type}</TableCell>
              
              <TableCell className="text-left">
                <Badge variant="outline" className="bg-gray-300">{order.priority.toUpperCase()}</Badge>
              </TableCell>
              <TableCell className="text-left">
                <Badge
                  variant={
              ...
              <TableCell className="text-left">
                <SheetTrigger asChild>
                  <Button variant="outline" className="bg-gray-200" onClick={()=>hendatOrderDetailAction(order.order_id)}>
  ...
  <div className="px-6 py-4 space-y-6 overflow-y-auto h-[calc(100vh-80px)] ">
    ...
    <section className="space-y-3">
      ...
      />
                  ...
                  <Requirments_info materials={ordersDetail?.materials} shade={ordersDetail?.order.shade} tooth_numbers={ordersDetail?.order.tooth_numbers}></Requirments_info>
                  
                  <File_info files={ordersDetail?.files}></File_info>
                  <Billing_info price={ordersDetail?.billing?.price}></Billing_info>
                  <Shipping address={ordersDetail?.shipping.address} method={ordersDetail?.shipping.method} note={ordersDetail?.order.design_notes}></Shipping>
                  </SheetContent> */}
              </TableCell>
            </TableRow>
            </Sheet>
